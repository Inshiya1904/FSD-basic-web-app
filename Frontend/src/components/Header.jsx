  ...
  return (
    <header className="fixed top-0 w-full z-50 bg-gradient-to-r from-[#FF6B6B] via-[#FFB347] to-[#FFD700] text-white shadow-lg">
      <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
        <Link to="/">
          <h1 className="text-xl md:text-3xl font-bold tracking-wide drop-shadow">
            Basti Ki Pathshala
          </h1>
        </Link>
        ...
        <nav className="hidden md:flex space-x-6 text-sm font-semibold">
          ...
          {!isAdmin ? (
            ...
            <button onClick={handleLogout} className="hover:text-black text-lg transition">Logout</button>
          )}
        </nav>
      ...
      {menuOpen && (
        <nav className="md:hidden bg-gradient-to-br from-[#FF6B6B] to-[#FFD700] border-t border-yellow-300 px-6 py-4 space-y-3 text-sm font-semibold">
          ...
          {!isAdmin ? (
            ...
            <button onClick={() => { handleLogout(); setMenuOpen(false); }} className="block hover:text-black">Logout</button>
          )}
        </nav>
      )}
    </header>
